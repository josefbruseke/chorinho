...
```json
{
  ...
  "scripts": {
    ...
    "local-ship": "bun abi-copy && bun codegen && bun build --network localhost && bun deploy-local",
    "test": "graph test -d",
...
```json
{
  "graph": "bun workspace @se-2/subgraph graph",
  "graphclient:build": "bun workspace @se-2/nextjs client",
  "subgraph:abi-copy": "bun workspace @se-2/subgraph abi-copy",
  "subgraph:build": "bun workspace @se-2/subgraph build",
  "subgraph:clean-node": "bun workspace @se-2/subgraph clean-node",
  "subgraph:codegen": "bun workspace @se-2/subgraph codegen",
  "subgraph:create-local": "bun workspace @se-2/subgraph create-local",
  "subgraph:local-ship": "bun workspace @se-2/subgraph local-ship",
  "subgraph:run-node": "bun workspace @se-2/subgraph run-node",
  "subgraph:stop-node": "bun workspace @se-2/subgraph stop-node",
  "subgraph:test": "bun workspace @se-2/subgraph test -d"
}
```
...
After running `bun graphclient:build`, import the generated client. Use TanStack Query (already available in SE-2) for data fetching:
...
> **`~~/.graphclient`** is the generated runtime artifact. It only exists after `bun graphclient:build`. The `.graphclient/` directory should NOT be committed — it's generated from `.graphclientrc.yml` and the GQL files.

## Gotchas & Common Pitfalls

**Docker must be running.** The local Graph Node, IPFS, and Postgres all run in Docker. If Docker isn't running, `bun subgraph:run-node` will fail.

**`bun deploy` must run before `bun subgraph:abi-copy`.** The ABI copy script reads from `deployedContracts.ts` which is generated by the deploy step. If you haven't deployed, there's nothing to copy.
...
**Graph Client artifacts must be regenerated after schema changes.** Run `bun graphclient:build` whenever you change the GraphQL schema or queries. The frontend imports from `~~/.graphclient` which contains generated types.
...
1. `bun chain` — start local blockchain
2. `bun deploy` — deploy contracts (generates `deployedContracts.ts`)
3. `bun subgraph:run-node` — start Docker Graph Node (keep this terminal open)
4. `bun subgraph:create-local` — register subgraph (once only)
5. `bun subgraph:local-ship` — copies ABIs, generates types, builds, and deploys
6. Visit `http://localhost:8000/subgraphs/name/scaffold-eth/your-contract/graphql` — test GraphQL queries
7. `bun graphclient:build` — generate frontend client artifacts
8. `bun start` — visit the subgraph page to see indexed data
9. `bun subgraph:test` — run Matchstick unit tests
...
3. `bun graph auth --studio <DEPLOY_KEY>`
4. `bun graph deploy --studio <SUBGRAPH_SLUG>`
