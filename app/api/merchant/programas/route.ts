...
const COLUNAS = "id, name, description, kind, base_benefit, cap_cents, product, starts_at, ends_at, joint, active";
  ...
  try {
    ...
    return NextResponse.json({
      ...
      programas: (meus ?? []).map(p => ({
        ...mapear(p),
        // O criador entra aceito no próprio programa.
...
export async function POST(request: NextRequest) {
  ...
  // M9: o teto de 32 bytes vinha do `bytes32` da cadeia. Sem ela, o que limita
  // o nome é a etiqueta na vitrine — e sessenta caracteres ainda cabem nela.
  if (nome.length < 2 || nome.length > 60) {
    return NextResponse.json({ erro: "o nome precisa ter de 2 a 60 caracteres" }, { status: 400 });
  }
  ...
  try {
    ...
    // O criador já entra aceito na própria pool: ele não precisa se convidar.
    if (conjunto) {
    ...
    return NextResponse.json({ ok: true, programa: mapear(programa) });
  } catch (e) {
    if (e instanceof ErroDeGestao) return NextResponse.json({ erro: e.message }, { status: e.status });
