...
type Resposta = {
  ...
  aviso?: string;
  /** A entrega do selo está fora do ar — ver `app/api/carteira/colecao/route.ts`. */
  entregaIndisponivel?: boolean;
  pecasIndisponiveis?: boolean;
};
...
export const MinhaColecao = () => {
   ...
   *
   * O carimbo cai no balcão, mas o selo não é entregue ali: é uma volta a mais
   * com o caixa parado e a fila esperando. A entrega acontece aqui, com o
   * cliente no sofá — e sem pedir um toque, porque exigir que a pessoa descubra
   * um botão para receber o que já é dela é atrito à toa.
   *
   * Três travas:
   *
   * - `entregaIndisponivel` corta na raiz. Enquanto a entrega está fora do ar,
   *   tentar é bater numa porta fechada uma vez por conquista, por sessão, e
   *   depois mentir para a pessoa que foi ela que teve azar.
   * - `jaTentadas` guarda quem já tentamos NESTA sessão. Sem isso, uma entrega
   *   que falha volta como `merecida` na recarga e o efeito tenta de novo, para
   *   sempre.
   * - `emCurso` serializa, para duas entregas não disputarem a mesma linha de
   *   `achievement_claims` — o `claim_ref` é único e a segunda voltaria como
   *   conflito.
   *
  ...
  useEffect(() => {
    if (dados?.entregaIndisponivel) return;
        ...
        <ul className="m-0 flex list-none flex-col gap-2 p-0">
          {aReceber.map(c => (
            <li key={c.id} className="flex flex-col gap-3 rounded-2xl border-2 border-primary bg-primary/5 p-4">
              ...
              {/* Com a entrega pausada o botão sai da tela: oferecer "tentar de
                  novo" para algo que vai falhar de novo é empurrar a culpa para
                  quem não tem nenhuma. */}
              {!dados.entregaIndisponivel && (
                <button
                  type="button"
              ...
              <span className="text-center text-xs opacity-70">
                {dados.entregaIndisponivel
                  ? "A entrega de selos está pausada. A conquista já é sua e aparece aqui assim que voltar."
                  : recebendo === c.id
                    ? "Estamos registrando. Pode deixar a tela aberta."
