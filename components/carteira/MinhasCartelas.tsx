...
type Resposta = { cliente: string | null; cartelas: Cartela[]; pontos: number };
...
export const MinhasCartelas = () => {
  ...
  useEffect(() => {
    ...
    const canal = supabaseBrowser()
      .channel(`cartelas:${cliente}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table: "stamp_balances_cache", filter: `customer_profile_id=eq.${cliente}` },
        () => void carregar(),
      )
      .subscribe();
