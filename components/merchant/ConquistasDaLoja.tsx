...
type Peca = { id: string; titulo: string; nivel: number };

type Dados = { loja: { nome: string }; conquistas: Conquista[]; pecas: Peca[] };
 ...
 * já tem. O lojista escreve a regra e pronto — não existe caminho em que alguém
 * ganhe sem ter cumprido, e ninguém precisa auditar entrega nenhuma depois.
 *
 * "Uma compra específica" é a exceção, e a tela diz isso na cara: ela depende de
 * o servidor atestar uma venda específica, porque o saldo guarda acumulado e
 * não valor por compra.
 */
    ...
    <div className="flex flex-col gap-5">
      <header>
        ...
        <p className="m-0 mt-1 text-sm opacity-75">
          Você escreve a regra; a conferência é feita na hora de entregar, contra os carimbos que a pessoa já tem.
          Ninguém recebe sem ter cumprido, e você não precisa conferir nada depois.
        </p>
      </header>
      ...
      >
        ...
        {criterio === 4 && (
          <p className="m-0 rounded-xl border border-warning bg-warning/10 p-3 text-xs font-semibold">
            Este é o único critério que não sai da cartela. Ela guarda o acumulado, não o valor de cada compra — aqui
            quem atesta a venda é o nosso servidor, na hora em que ela acontece.
          </p>
        )}
        ...
        <fieldset className="flex flex-col gap-2 rounded-xl bg-base-200 p-3">
          ...
          {/* Sem peça ativa, o seletor seria uma caixa com uma opção só. Vale
              mais o caminho até a coleção do que o controle vazio. */}
          {dados.pecas.length === 0 ? (
            <p className="m-0 text-xs font-normal opacity-70">
              Sua coleção ainda não tem peça ligada para prometer aqui.{" "}
              <Link href="/painel/colecao" className="link font-bold">
                Crie uma peça
              </Link>{" "}
              e ela aparece nesta lista.
            </p>
          ) : (
            <label className="flex flex-col gap-1 text-xs font-semibold opacity-75">
