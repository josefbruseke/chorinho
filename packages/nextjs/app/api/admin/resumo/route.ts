...
export async function GET() {
  ...
  const [
    ...
    admin.from("sales").select("stamps_issued, customer_wallet").eq("status", "confirmada").gte("created_at", desde30),
    admin
      .from("establishments")
      .select("id, name")
      .eq("status", "pendente")
  ...
  const clientesUnicos30d = new Set((carimbos30d.data ?? []).map(v => v.customer_wallet)).size;
  ...
  return NextResponse.json({
    ...
    atencao: {
      ...
      lojasSemOnchainId: (listaLojasSemOnchainId.data ?? []).map(l => ({ id: l.id, nome: l.name })),
