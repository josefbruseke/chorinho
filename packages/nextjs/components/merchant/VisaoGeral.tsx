...
type Resumo = {
  loja: { nome: string; onchainId: number | null; papel: string };
  hoje: Janela;
  mes: Janela;
  terminais: { ativos: number; limite: number };
  recompensas: number;
  rede: {
    assinatura: { ativa: boolean; plano: number; venceEm: number };
    regra: { ativa: boolean; pisoDeTicketCentavos: number; centavosPorCarimbo: number; pontosPorCarimbo: number };
  } | null;
 ...
 * Depois o que está travando a operação — configuração faltando, assinatura
 * vencida — porque é a única coisa que exige ação dele. O resto é consulta.
 */
    ...
    <div className="flex flex-col gap-6">
      <header className="flex flex-col justify-between gap-4 border-b border-base-300 pb-5 sm:flex-row sm:items-center">
        <div className="min-w-0">
          ...
          <p className="m-0 mt-1 text-sm opacity-75">
            {dados.rede?.assinatura.ativa
              ? "Assinatura ativa — o balcão está carimbando"
              : dados.rede
                ? "Assinatura vencida"
                : "Estado da rede indisponível agora"}
          </p>
        </div>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Cartao rotulo="Faturado" valor={formatarCentavos(dados.hoje.centavos)} />
        </div>
      </section>
      ...
      <section>
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          ...
          <Cartao rotulo="Faturado" valor={formatarCentavos(dados.mes.centavos)} />
        </div>
      </section>
      ...
      {dados.rede?.regra.ativa && (
        <section className="rounded-2xl border border-base-300 bg-base-100 p-4">
          <h2 className="m-0 text-xs font-bold uppercase tracking-wide opacity-70">Regra de carimbo em vigor</h2>
          <p className="m-0 mt-1.5 text-base">
            Cada <strong className="font-mono">{formatarCentavos(dados.rede.regra.centavosPorCarimbo)}</strong> de
            compra valem <strong>1 carimbo</strong>, a partir de{" "}
            <strong className="font-mono">{formatarCentavos(dados.rede.regra.pisoDeTicketCentavos)}</strong>.
          </p>
        </section>
      )}
    </div>
