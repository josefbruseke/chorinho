 ...
 * amarra a carteira do cliente ao balcão que o leu. Não vai ao banco — é
...
export const abrirAtendimento = (carteira: string, balcaoId: string) => {
  const corpo = Buffer.from(
    JSON.stringify({ w: carteira.toLowerCase(), b: balcaoId, e: Math.floor(Date.now() / 1000) + VALIDADE_SEGUNDOS }),
  ).toString("base64url");
  return `${corpo}.${assinar(corpo)}`;
};

export type Atendimento = { carteira: string; balcaoId: string };
...
export const lerAtendimento = (token: string): Atendimento | undefined => {
  try {
    ...
    return { carteira: dados.w, balcaoId: dados.b };
  } catch {
    return undefined;
  }
};
