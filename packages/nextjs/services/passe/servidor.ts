...
const assinar = (endereco: string, nonce: string, expira: number) =>
  createHmac("sha256", segredo()).update(`${endereco.toLowerCase()}|${nonce}|${expira}`).digest("base64url");

export const assinarPasse = (endereco: string, nonce: string, expira: number): PassePayload => ({
  v: PASSE_VERSAO,
  a: endereco.toLowerCase(),
  n: nonce,
  e: expira,
  s: assinar(endereco, nonce, expira),
});
